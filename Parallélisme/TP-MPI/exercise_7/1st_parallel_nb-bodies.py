...
a= rank* nbbodies//size
b= (rank+1)* nbbodies//size

start_time = MPI.Wtime()
for t in range(0, NBSTEPS):
	forces = [[0,0] for _ in range(nbbodies)]
	for i in range(a, b):		
		for j in range(0, nbbodies):
			[fx,fy] = forces[i]
			[dfx, dfy] = interaction(world[i], world[j])
			forces[i] = [fx+dfx, fy+dfy]

	for i in range(a, b):
		world[i] = update(world[i], forces[i])
	
	world = comm.allreduce(world[a: b], op=MPI.SUM)
	#if(rank==0):
	#	displayPlot(world)
end_time = MPI.Wtime()

print("in=", end_time-start_time, " seconds", " signature=", signature(world))
#12 elements, 1000 iterations: in= 0.11677241325378418  seconds  signature= 3.943398426376769e+17
#1024 elements, 10 iterations, 1 process: in= 9.513486084  seconds  signature= -1.6746669265218414e+20

#1024 elements, 10 iterations, 2 processes: in= 4.978469973  seconds  signature= -1.6746669265218414e+20

#1024 elements, 10 iterations, 4 processes: in= 3.076994057  seconds  signature= -1.6746669265218414e+20
