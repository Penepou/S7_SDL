...
start = rank* nbbodies//size
end = (rank+1)* nbbodies//size
...
for t in range(NBSTEPS):
    ...
    for i in range(start, end):
        for j in range(i, nbbodies):
            [dfx, dfy] = interaction(world[i], world[j])
            [fx, fy] = force[i]
            force[i] = [fx+dfx, fy+dfy]
            [fx, fy] = force[j]
            force[j] = comm.bcast([fx-dfx, fy-dfy], root=rank)
    for i in range(start, end):
        world[i] = update(world[i], force[i])
    world = comm.allreduce(world[start:end])
    #if rank == 0:
     #   displayPlot(world)
end_time = time.time()
...
#1024 elements, 10 iterations, 1 process: in= 15.658934831619263  seconds  signature= -1.6746669265218414e+20


#1024 elements, 10 iterations, 2 processes: in= 19.136680364608765  seconds  signature= -1.677501044542709e+20


#1024 elements, 10 iterations, 4 processes: in= 18.055004596710205  seconds  signature= -1.6826068194580785e+20
