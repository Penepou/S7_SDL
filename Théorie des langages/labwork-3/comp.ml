...
let rec comp_expr e = match e with
	...
	| NEG (e) -> let (v, q) = comp_expr e in let r = new_reg () in (r, q @ [ SUB (r, 0, v) ])
	| VAR (x) -> let v = new_reg () in (v, [ SET (v, x) ])
	| BINOP (op, e1, e2) ->  let (v1, q1) = comp_expr e1 in let (v2, q2) = comp_expr e2 in
			let r = new_reg () in (r, q1 @ q2 @ [ match op with
				| OP_ADD -> ADD (r, v1, v2)
				| OP_SUB -> SUB (r, v1, v2)
				| OP_MUL -> MUL (r, v1, v2)
				| OP_DIV -> DIV (r, v1, v2)
